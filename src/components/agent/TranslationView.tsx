...
export function TranslationView() {
  ...
  // Auto trigger translation on page change
  useEffect(() => {
    if (
      currentText &&
  ...
  }, [currentPage, currentText, currentTranslation, translatePage]);
          ...
          {hasContent && (
            <div className="flex items-center gap-1 pl-1 border-l border-zinc-200">
              <button
                ...
                title="重新翻譯此頁"
                className="p-1.5 text-zinc-400 hover:text-zinc-700 rounded-lg hover:bg-zinc-100 transition-colors disabled:opacity-30"
              >
        ...
        <div className="max-w-2xl mx-auto">
          {!file ? (
            <div className="h-full min-h-[420px] flex flex-col items-center justify-center text-center text-zinc-400 text-xs py-24">
              ...
              <p>請在左側拖入 PDF 論文或在終端機輸入 /solo-master &lt;pdf&gt;，原文與即時翻譯將在此對照呈現。</p>
            </div>
          ) : isError ? (
            ...
            <div className="prose prose-zinc max-w-none text-zinc-800 text-sm leading-relaxed select-text">
            ...
            <div className="flex flex-col items-center justify-center py-28 text-center text-zinc-400 text-xs">
              ...
              <button
                ...
                className="px-3.5 py-1.5 rounded-lg bg-zinc-100 hover:bg-zinc-200 text-zinc-700 font-medium transition-colors"
              >
                開始翻譯此頁
              </button>
            </div>
